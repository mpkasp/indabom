...
class PartForm(forms.Form):
    ...
    number_item = forms.CharField(max_length=4, label='Part Number', required=False)
    number_variation = forms.CharField(max_length=2, label='Part Variation', required=False)
    ...
    new_manufacturer = forms.CharField(max_length=128, label='Create New Manufacturer', required=False)
    ...
    def clean(self):
        ...
        if mfg and new_mfg:
            raise forms.ValidationError(('Cannot have a manufacturer and a new manufacturer'), code='invalid')
        elif new_mfg:
...
class AddSubpartForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['assembly_subpart'].queryset = Part.objects.filter(organization=self.organization).order_by('number_class__code', 'number_item', 'number_variation')
